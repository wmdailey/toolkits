...
function usage() {
# usage
        echo "Usage: sudo $(basename $0) [OPTION]"
        exit 
}
...
DESCRIPTION
        ...
        -h, --help
                Help page
        -b, --backup
                Backup the databases to /tmp
	-d, --delete
		Delete the databases and uninstall postgresql
	-c, --create
...
# The second is configured to specific databases 
	...
	check_file ${work_dir}/conf/custom_postgresql.conf
	cp ${work_dir}/conf/custom_postgresql.conf ${pg_data}/postgresql.conf
	cp ${work_dir}/conf/custom_pg_hba.conf ${pg_data}/pg_hba.conf
	chown postgresql:postgresql ${pg_data}/postgresql.conf
	chown postgresql:postgresql ${pg_data}/pg_hba.conf
}
